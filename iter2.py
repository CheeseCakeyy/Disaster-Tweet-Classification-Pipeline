...
def feature_creation(df):
    ...
    df['text_combined'] = df['keyword'] + ' ' + df['text'] #boosts signal/ good with tf-idf
    #cleaning the text 
...
preprocessor = ColumnTransformer(
    transformers=[
        ...
        ('scaler',StandardScaler(),remaining_cols)
    ],
    remainder='drop'
)
...
'''val_f1_score,precision,recall with Log_reg:  0.7671009771986971 0.8205574912891986 0.7201834862385321 0.8122127380170716
val_f1_score,precision,recall with SVM:  0.7754777070063694 0.8089700996677741 0.7446483180428135 0.81483913328956
the f1 score remained same for logReg but growth in recall was seen with a slight drop in precision,expected tradeoff'''
